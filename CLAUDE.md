...
- Latest (2026-06-16): **Analisi current-year charts scoped to the selected year** — "Andamento Risparmio" and "Trend per Categoria" now take an optional `scopeYear` and follow the period selector: current year in "Anno Corrente" (12m/24m/Tutto toggle hidden), the chosen past year in "Anno" (newly shown — previously hidden), full history + toggle only in "Storico". Fixes the bug where "Anno Corrente" pulled in prior-year months. Also moved those charts' target line / below-target tint / hover cursor from hardcoded green/red/grey to theme tokens (`--positive`/`--destructive`/`--muted`).
...
**Last updated**: 2026-06-16 (Analisi "Andamento Risparmio" + "Trend per Categoria" scoped to the selected year via `scopeYear`; both rendered in "Anno" mode; chart accent colors moved to theme tokens. See Current Status → Latest.)

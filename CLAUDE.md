...
- Latest implementation (2026-05-08, session jiggly-deer): **Mesi+/- denominator display in benchmark comparison table**. Cells now show "X/Y" format (e.g. "29/40" for portfolio, "30/41" for benchmark) with the denominator in muted text. Added `totalMonths: number` field to `BenchmarkMetrics` (= `returns.length`). Footnote updated to explain that the portfolio denominator is N−1 because the first snapshot is baseline only. Pure display change — no logic modified. Key file: `components/performance/BenchmarkComparisonChart.tsx`.
- Previous (2026-05-07, session curried-sutherland): **ECB deposit facility rate auto-sync via FRED API**. Sharpe/Sortino in "Confronto con Portafogli Modello" now use `effectiveRiskFreeRate` = arithmetic mean of monthly ECB rates over the exact evaluation period, fetched from FRED series ECBDFR and cached in Firestore `ecb-rate-cache/deposit-rate` (TTL 7d, Admin SDK write only). New files: `lib/server/ecbRatesService.ts` (FRED fetch + carry-forward + Firestore read/write), `app/api/benchmarks/ecb-rates/route.ts` (auth-gated GET, graceful degradation to stale cache or 503), `lib/hooks/useEcbRates.ts` (React Query, 6h staleTime, enabled when section open). `BenchmarkComparisonChart` computes `effectiveRiskFreeRate` via `computePeriodAverageRiskFreeRate`, overrides portfolio Sharpe in the table (KPI card keeps user rate), and renders a dynamic footnote showing BCE rate or user fallback. Daily cron refreshes ECB cache non-blocking. Env var: `FRED_API_KEY` (.env.local + Vercel). **⚠️ No longer requires manual ECB rate table updates** — now auto-synced from FRED.
- Previous (2026-05-05, session add-portfolio-metrics): **Portfolio metrics + new benchmarks in Rendimenti**. Expanded "Confronto con Portafogli Modello" table from 2 to 11 columns: TWR ann., Crescita tot., Volatilità, Sharpe, Sortino, Calmar, Max Drawdown, Miglior/Peggior mese, Mesi+/-. All metrics computed from the same filtered monthly return series (apples-to-apples); portfolio Volatilità/Sharpe/Max DD use pre-computed cashflow-adjusted props for KPI consistency. Added 2 new model portfolios: `permanent-portfolio` (25% VTI/TLT/GLD/SHY, Harry Browne) and `acwi-100` (100% ACWI, equity puro). Fixed hook count updated from 4 to 6 in `BenchmarkComparisonSection`. Key files: `lib/constants/benchmarks.ts`, `components/performance/BenchmarkComparison{Chart,Section}.tsx`, `app/dashboard/performance/page.tsx`.
...
**Last updated**: 2026-05-08 (session jiggly-deer — Mesi+/- denominator display in benchmark comparison table)

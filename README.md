...
- **Analisi tab**: unified period analysis with a three-state selector (current year / specific year + optional month / full history). **Top Expenses block** shows the largest individual expenses for the selected period sorted by amount (top 5 by default, expandable); resets automatically when the period changes. 5-layer Sankey diagram, 4-level drill-down with breadcrumb navigation and hierarchical back-navigation, monthly and annual trend charts in a collapsible section. All drill-down transaction lists show an aggregated total row (count + sum) at the bottom. All charts respect the "history start year" preference from Settings

...
- **Analisi page** (`/dashboard/analisi`): dedicated standalone page for cashflow analysis with a three-state period selector (current year / specific year + optional month / full history), saved in the page link so refreshing or sharing keeps your view. The key numbers stay up front: a KPI trio (income / expenses / net savings, with a 12-month average shown alongside a deficit month for context), **Anomaly detection** flagging categories with unusually high spending vs the 6-month rolling average (tap a chip to jump to the Sankey drill-down), a 5-layer Sankey diagram with a fully clickable drill-down breadcrumb, and a **Top Expenses block** (top 5 by amount, expandable). Deeper analysis — **Year-over-year comparison**, **Savings Rate Trend**, **per-category sparkline grid**, and (in full-history mode) an **"Andamento nel Tempo"** section with long-term income/expenses/net-savings and per-category charts — collapses behind a "Mostra dettaglio" toggle so it's there when you want it without crowding the page. Ranked composition-list drill-downs (category → sub-category → transactions) with aggregated total rows. All charts respect the "history start year" preference from Settings
